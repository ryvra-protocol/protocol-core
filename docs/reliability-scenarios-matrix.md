...
| Late failure after terminal success | Terminal settled state is not corrupted by stale failure callback | State remains `settled`; reward eligibility stays true; stale audit event emitted | `integration-sandbox/test/pay-reliability.spec.ts` | pass |
